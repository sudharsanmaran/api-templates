...
all_templates = {**google_templates, **microsoft_templates, **twitter_templates}
...
async def apply_form(data: Form):
    """Applies the form."""
    result = await all_actions.get(data.data.action_name)(data.model_dump())
...
async def fill_form(data: Form) -> ActionsForm:
    """Fills the form."""
    result = await all_fillforms.get(data.data.action_name)(data.model_dump())
    if result.data.status == Status.error:
        return JSONResponse(content=result.model_dump(), status_code=400)
    return JSONResponse(content=result.model_dump(), status_code=200)
